...
void sin_expanded(double* sinx, const double* x) {
  // The Vec class is defined in the file intrin-wrapper.h
  typedef Vec<double,4> Vec4;
  Vec4 x1, x2, x3, x5, x7, x9, x11;
  x1  = Vec4::LoadAligned(x);
...
double err(double* x, double* y, long N) {
  ...
  //for (long i = 0; i < N; i++) error += fabs(x[i]-y[i]);
  return error/N;
}
...
int main() {
  ...
  double* sinx_expanded = (double*) aligned_malloc(N*sizeof(double));

  for (long i = 0; i < N; i++) {
    x[i] = (rand()-0.5) * M_PI/2; // [-pi/4,pi/4]
    sinx_ref[i] = 0;
    sinx_taylor[i] = 0;
    sinx_intrin[i] = 0;
    sinx_vector[i] = 0;
 ...
 tt.tic();
  for (long rep = 0; rep < 1000; rep++) {
    for (long i = 0; i < N; i+=4) {
      sin_expanded(sinx_vector+i, x+i);
    }
  }
  printf("Expanded time:    %6.4f      Error: %e\n", tt.toc(), err(sinx_ref, sinx_expanded, N));
